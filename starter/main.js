...
h1Ele.textContent = "The Best To Do List"
...
let itemNameField = document.getElementsByName("itemName")[0]

// Step 3b - The Due Date field
let dueDateField = document.getElementsByName("dueDate")[0]
...
let addNewBtn = document.querySelector("#addNewItem")
...
addNewBtn.addEventListener("click", function(event) {
  h1Ele.style.backgroundColor = "#EBD5B3"
  h1Ele.textContent = "The Best To Do List"
  ...
  if (itemNameField.value == "")
  {
    ...
    h1Ele.textContent = "Please Enter A Name"
  ...
  {
    h1Ele.style.backgroundColor = "red"
    h1Ele.textContent = "Please Enter A Due Date"
    return false
  }
  ...
  let itemEntryCell = content.querySelector(".item-entry")
  itemEntryCell.textContent = itemNameField.value

  // Step 4f - Using DOM walking, access the item due date cell
  // and store the current due date value
  let dueDateCell = content.querySelector(".item-due-date")
  dueDateCell.textContent = dueDateField.value

  // Step 4g - Using DOM walking, access the item delete button
  // and make the onclick property equal to a function definition
  // named removeItem
  let itemDeleteBtn = content.querySelector(".item-delete")
  //itemDeleteBtn.onclick = function removeItem(event) {}
  //itemDeleteBtn.onclick = removeItem

  // Step 4h- Using DOM walking, access the item edit button
  // and make the onclick property equal to a function definition
  // named editItem
  let itemEditBtn = content.querySelector(".item-edit")
  //itemEditBtn.onclick = editItem
  ...
  // into a new node:
  // moved this down here because it doesn't work unless it's at the end
...
function editItem(event)
{
  ...
  let editName = event.target.closest('tr').children[0]
  let editDate = event.target.closest('tr').children[1]
  ...
  editDate.setAttribute('contenteditable', true)

  // Step 6c - Trigger focus on the element
  editName.focus()

    // Step 6d - Create an eventlistener on the blur event
  editName.addEventListener('blur', function(event){
    ...
    editDate.addEventListener('blur', function(event){
    editDate.setAttribute('contenteditable', false)
  })
}
